...
* Every query has sql injection protection ! 
* Keep Safe ! And Enjoy !
* October@2021
*/
	...
	*/
	...
	private function queryOrderBy()
	{
		if ($this->order_by_columns != false) {
			$this->query_orderby = ' order by ' . $this->removeSingleQuote($this->order_by_columns);
		}
	}
	...
	private function queryUpdate($val)
	{
		...
		foreach ($val as $key => $value) {
			...
			$key = $this->removeSingleQuote($key);
			...
			foreach ($this->where_not as $key => $value) {
				...
				if ($i == 0 && ($count_where_not == 1)) {
					...
					if ($i == ($count_where_not - 1)) {
						...
						$where_not .= $key . ' <> ' . $value . $this->operator;
					}
				}
				$i++;
			}
			...
			foreach ($this->where_like as $key => $value) {
				...
				if ($i == 0 && ($count_where_like == 1)) {
					...
					if ($i == ($count_where_like - 1)) {
						...
						$where_like .= $key . ' like ' . $value . ' '.$this->operator.' ';
					}
				}
				$i++;
			}
	...
	private function queryLimit()
	{
		if ($this->limit_number != false) {
			$this->query_limit = ' limit ' . $this->removeSingleQuote($this->limit_number);
		}
	}
			...
			foreach ($this->wheres as $key => $value) {
				...
				if ($i == 0 && ($count_where == 1)) {
					...
					if ($i == ($count_where - 1)) {
						...
						$wheres .= $key . '=' . $value . ' '.$this->operator.' ';
					}
				}
				$i++;
			}
